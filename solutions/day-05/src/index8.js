...
class Banderas extends React.Component {
  ...
  render() {
    //const pais=countriesData[Math.floor(Math.random()*countriesData.length)]
    const {
      name, capital,language,population,flag,currency
    }=this.props.pais
    ...
    return(
      <div className="bandera">
        ...
        <h3>{name}</h3>
        <div className="text flags">
          <p><strong>Capital: </strong>{capital}</p>
          <p><strong>language: </strong>{language}</p>
          <p><strong>Population: </strong>{population}</p>
          <p><strong>Currency: </strong>{currency}</p>
        </div>
        <Button text="Select country" onClick={this.props.changeCountry} style={buttonStyles}/>
      </div>
    )
  }
}
...
class App extends React.Component {
  state = {
    ...
    pais:countriesData[Math.floor(Math.random()*countriesData.length)]
  }
  showDate = (time) => {
    const months = [
      'January',
  ...
  changeCountry=()=>{
    this.setState({pais:countriesData[Math.floor(Math.random()*countriesData.length)]})
  }
